...
def get_all_features(des_list, num_words):
    ...
    for i in range(len(des_list)):
        if des_list[i] != []:
            allvec[i] = des2feature(centures=centres,
                                    des=des_list[i],
                                    num_words=num_words)
    return allvec
...
# test
# 或者文件中的所有图像
img_paths = get_img_path('/home/gjx/visual-struct/dataset/mix')
num_words = 6  # 聚类中心数
# 得到质心， 和所有样本的sift特征。
centres, des_list = getClusterCentures(img_paths=img_paths,
                                       num_words=num_words,
                                       dataset_matrix=None)
# 获取所有训练样本的特征。
...
# 对新来的样本和已有样本进行相似度检索。
path = '/home/gjx/visual-struct/dataset/mix/cat5.jpg'
