...
"""
'''
https://www.cnblogs.com/lzhu/p/8806048.html
词袋模型BOW+SVM 目标识别
...
class BOW(object):
    ...
    def fit(self, train_path, k):
        ...
        for i in range(length):
            bow_kmeans_trainer.add(
                self.sift_descriptor_extractor(self.path(pos, i)))
            bow_kmeans_trainer.add(
                self.sift_descriptor_extractor(self.path(neg, i)))
    ...
    def sift_descriptor_extractor(self, img_path):
        ...
        return self.descriptor_extractor.compute(
            im, self.feature_detector.detect(im))[1]
    ...
    def bow_descriptor_extractor(self, img_path):
        ...
        '''
        im = cv2.imread(img_path, 0)
        return self.bow_img_descriptor_extractor.compute(
            im, self.feature_detector.detect(im))
...
if __name__ == '__main__':
    ...
    if test_results[0]:
        cv2.putText(dog_img, 'Dog Detected', (10, 30), font, 1, (0, 255, 0), 2,
                    cv2.LINE_AA)
